...
### Task 6: 얇은 월 URL 색인 기준 — ✅ **결정됨 (2026-08-31, 픽서님 위임)**

> **결정: B안 축소판을 실행한다.**
> - **휴일도 이벤트도 둘 다 없는 월 페이지에만** `noindex` + 사이트맵 제외. 실측 45개 × 5언어 = **225 URL**. 착수 시 재측정하고 45에서 크게 벗어나면 멈춘다.
> - **지역 허브는 하나도 건드리지 않는다**(=A안). 314개 중 143개가 글 2편 이하지만, 지역 허브에는 실제 장소 목록·고유 소개문·FAQ가 있어 "아무것도 없는" 페이지가 아니다. 그리고 이 부류가 2026-07-26에 92편 삭제로 트래픽 40%를 날린 바로 그 부류다.
> - **페이지는 계속 생성한다.** 라우트에서 빼면 404가 되고 그게 07-26 사고의 재판이다. `noindex` 만 붙인다.
> - **상수 하나로 되돌아가게** 만든다.
> - **기대치를 정직하게**: 이 225 URL은 이미 노출이 거의 0이다(tools -92%). 효과는 트래픽이 아니라 **신호 위생**이고, 크게 오를 거라 기대하지 않는다. C안(최대 940 URL)을 고르지 않은 이유는 색인이 07-25부터 얼어 있어 **뺀 URL이 언제 돌아올지 모르기 때문**이다.

**원래 결정이 필요했던 이유:** 이건 코드 문제가 아니라 **정책 결정**이다. 실행하면 최대 **715개 허브 URL + 최대 171개 월 URL**이 색인에서 빠진다. 2026-07-26에 사진 없는 글 92편을 지웠다가 트래픽의 40%를 날린 전례가 있고, 지금은 색인이 07-25부터 얼어 있어 **뺀 URL이 언제 돌아올지 알 수 없다.**
...
| 안 | 내용 | 잃는 URL | 판정 |
|---|---|---|---|
| A | 색인 안 건드리고 콘텐츠를 채운다 | 0 | 지역 허브에는 **채택** |
| B | 휴일·이벤트 둘 다 없는 45개 월 페이지만 `noindex` | 225 | 월 페이지에는 **채택** |
| C | 지역 허브 기준 4편 + 월 페이지 전면 | 최대 940 | **기각** — 색인이 얼어 있어 뺀 URL의 복귀를 보장할 수 없다 |

**구현 형태:**
- `src/lib/thin-page-policy.mjs` (+test) 에 판정 함수 하나 — `isIndexableMonthPage({hasEvents, hasHolidays})`. **`isIndexableRegionHub` 은 만들지 않는다** — 지역 허브는 판정 대상이 아니라는 결정을 코드에도 남긴다.
...
- [ ] **Step 1: 45개를 재측정한다** — `docs/superpowers/plans/2026-08-31-thin-inventory.json` 생성에 쓴 것과 같은 방식(빌드 산출물에서 `wtg-ev` 유무 + `wtg-hol` 안의 `wtg-list` 유무). **45에서 크게 벗어나면 멈추고 보고한다.**
- [ ] **Step 2: `isIndexableMonthPage` 와 양방향 테스트를 먼저 쓴다** — 둘 다 없으면 false, 하나라도 있으면 true, 둘 다 있으면 true.
- [ ] **Step 3: 월 페이지 라우트에 `noindex` 를 연결하고 사이트맵에서 뺀다** (기존 `NOINDEX_SLUGS` 재사용)
- [ ] **Step 4: 빌드 후 세 가지를 확인한다** — ①사이트맵 총 URL이 정확히 225 줄었을 것 ②그 페이지들이 사용자에게는 여전히 **200** ③`noindex` 가 5개 언어 전부에 붙었을 것
- [ ] **Step 5: Task 0~5와 분리해서 별도 커밋·별도 푸시** (색인에 영향이 있는 유일한 변경이므로 되돌릴 때 깔끔하게)

---

### Task 7: 기존 1,349편의 사이트 단위 균일성 — ⏸️ **09-21 이후로 보류 (2026-08-31 결정, 픽서님 위임)**

> **결정: 지금 하지 않는다. 09-21 재제목 파일럿 판정을 보고 정한다.**
>
> 소심해서가 아니라 **이미 있는 데이터와 충돌**하기 때문이다. 메모리 `wander-atlas-retitle-experiment` 참조:
>
> 1. **"전면 다양화"는 08-31에 이미 실측으로 철회된 권고다.** 노출 상위 페이지의 평균순위가 77.8위·56.0위 — 8페이지짜리 순위에서는 제목을 아무리 잘 써도 사람이 볼 일이 없다. 최근 2주 사이트 전체 노출 1,862에 **클릭 6**이라 CTR을 개선할 신호 자체가 없다. 지금 이걸 되살리면 회귀다.
> 2. **09-01에 재제목 파일럿 6편이 나갔고 대조군 3편이 지정돼 있다**(`provence-lavender-sunflower-field`, `madrid-barcelona-the-weeknd`, `hanoi-…-miss-world-2026`). **판정일 09-21.**
> 3. **09-10에 코호트 판정**이 예약돼 있다.
>
> 지금 100편이든 1,349편이든 손대면 **두 판정을 동시에 오염시킨다.** 파일럿이 답하려던 질문("제목이 지렛대인가")을 스스로 못 읽게 만드는 셈이다.
>
> **재개 조건 (09-21 파일럿 판정):**
> - 파일럿 6편의 CTR이 대조군 방향으로 움직였다 → 1페이지에 오른 글로 **범위를 넓힌다**. 도구는 이미 있다(`scripts/retitle-post.mjs` — 제목·설명만 5개 언어 + srcHash 재스탬프로 전체 재번역 회피).
> - 판정 불가 / 효과 없음 → **제목 축은 접는다.** 분량·형태 다양화는 신규 발행(`scripts/lib/writer.mjs:152-170`, 08-28부터 가동)에 맡기고 소급 재작성은 하지 않는다.
>
> 어느 쪽이든 **em dash 10,657개 일괄 치환은 단독으로 돌리지 않는다** — 본문을 건드리는 순간 번역이 재큐되므로, 본문을 고칠 이유가 따로 생겼을 때 같은 배치에 태운다.

**아래는 판정일에 다시 읽을 근거 자료다 (실행 지시가 아니다).**
...
**비용 구조 (판정일에 필요한 숫자):** 본문이든 제목·설명이든 고치면 `srcHash` 가 바뀌어 번역 4개가 재생성된다. 1,349 × 4 = 5,396편이 한 번에 도는 건 청구서가 크다. 다만 **제목·설명만이라면 공짜로 할 수 있다** — `scripts/retitle-post.mjs` 가 5개 언어 제목·설명을 함께 바꾸고 srcHash를 재스탬프해서 전체 재번역을 회피한다. 그게 그 도구의 존재 이유다. 본문(H2 개수·분량·em dash)은 그 경로가 없다.

**그리고 08-31 결론은 바뀌지 않았다:** 병목은 여전히 **백링크(권위)**다. 균일성이 원인이라는 건 증명된 게 아니라 추정이다.

- [ ] **Step 1 (09-21): 재제목 파일럿 6편 vs 대조군 3편의 CTR을 GSC로 잰다.** 기준선은 07-20~08-29 6주 — 파일럿 노출 992·클릭 6·CTR 0.6% / 대조군 노출 151·클릭 7·CTR 4.6%.
- [ ] **Step 2 (09-21): 위 재개 조건에 따라 범위를 정하거나 제목 축을 접는다.** 노출이 얇아 무판정으로 끝날 수 있음은 파일럿 설계 때 이미 인정한 바다 — 무판정이면 접는 쪽이다.
...
1주차 (승인 불필요, 바로 실행)
  ...
  Task 6  얇은 월 페이지 noindex  ← 결정됨(B안 축소판, 225 URL). 색인 영향이 있으니 별도 커밋·별도 푸시

09-21 이후
  Task 7  기존 코퍼스 균일성      ← 보류 결정. 재제목 파일럿 판정을 깨뜨리지 않기 위해서다
```

**실행은 2026-09-07(월) 09:00 KST 예약 작업 `seo-remediation-0907` 이 자동으로 시작한다.** 픽서님이 08-31에 "알림 주고 알아서 시작해라, 결정 2건도 네가 판단해라"라고 위임했다. 예약 작업 파일: `C:/Users/user/.claude/scheduled-tasks/seo-remediation-0907/SKILL.md`
