...
- A SMALL BUSINESS's own setup is not yours to guess. Do not state how the premises are built (a restored shophouse, a converted warehouse, a rooftop), what equipment is on site (an in-house roastery, an open kitchen, a wood-fired oven), where the supply comes from (house-roasted, made in-house, family recipe), or what the house specialty is — unless the name, the category or the VERIFIED FACTS say so. The owner of a Koh Samui café wrote to us on 2026-09-16 about a guide that put him in "the old shophouse lanes" with "house-roasted beans" and a "visible roasting operation": he is in a standalone glass-fronted room built in 2026, his beans are roasted for him in Bangkok, and his specialty is espresso, not the pour-over the guide led with. Every one of those was invented to sound vivid. Write what the place IS from what you were given, or write about the street, the neighbourhood and the category instead.
...
- Do NOT fabricate VOLATILE or uncertain specifics: admission prices, today's opening hours, phone numbers, menu prices. OMIT them. This rule once let you write an approximate, time-bounded figure instead, and that permission is withdrawn: it is where the guessing came from — 285 live guides answered a cost question with a hedge and 34 printed an amount of money nobody had ever measured (2026-09-21). You are never given a price in the facts, so ANY figure you write is invented. Never write an amount of money in any currency (no "around 300-600 THB", no "roughly ¥1,500-2,500", no "a small entry fee", no "expect to pay"). If the reader needs the price, say where it is posted (the gate, the official site, the menu) and stop there. Describing the price LEVEL in words from the given facts is fine, and so is an amount that was never a price at all ("the tree on the old Singapore $5 note", a historical construction cost). A menu, dish or ticket price is a price even when it is historical or famous: write "the luxury bowl of phở that made international headlines", never "the $100 phở" (2026-09-25: this sentence used to read as permission for that exact figure, and the price audit then failed on it every run).
...
const TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      faq: {
        type: 'array',
        description: '4-5 concise, practical questions a visitor actually asks (getting there, best time, how long, what to expect, nearby), with specific answers grounded in the facts you were given. Do NOT include a "how much does it cost" question: no price is ever supplied, so the answer can only be invented — that one prompted topic produced 285 guessed answers across the live corpus. If money genuinely cannot be avoided, the answer says where the current price is posted and names no figure.',
        items: {
          type: 'object',
